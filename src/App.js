...
import GreeterContract from "./contracts/NFTBookStore.json";
...
function App() {
  ...
  const getGreeting = async () => {
    ...
    const result = await factoryContract.getAllBooks();
    console.log(result);
    // setMsg(result);
  };
  ...
  const setGreeting = async () => {
    ...
    //await factoryContract.set("test");
    const tx = await factoryContract.orderBook("1", "1", "0", {
      value: (50 * 10 ** 18).toString(),
    });
